...
def get_full_route(routeList):
    route = []
    for point in routeList:
        direction = (list(point))[0]
        amount = int("".join((list(point))[1:]))
        for i in range(amount):
            if not route:
                route.append(calculate_x_y(direction))
            else:
                route.append(calculate_x_y(direction, route[-1]))
    
    return route

def calculate_steps(routeList, intersection):
    route = []
    amountList = []
    for point in routeList:
        direction = (list(point))[0]
    ...
    total = 0
    for steps in amountList:
        total += steps

    return total

if __name__ == '__main__':
    route1 = set(get_full_route(input01))
    route2 = set(get_full_route(input02))
    intersection = list(route1.intersection(route2))
    inputStepList01 = []
    inputStepList02 = []
    stepBothLines = []
    for intersectionItem in intersection:
        input01Steps = calculate_steps(input01, intersectionItem)
        input02Steps = calculate_steps(input02, intersectionItem)
        print("{} input01: {}".format(intersectionItem, input01Steps))
        print("{} input02: {}".format(intersectionItem, input02Steps))
        inputStepList01.append(input01Steps)
        inputStepList02.append(input02Steps)

    for x in range(0, len(inputStepList01)):
        stepBothLines.append(inputStepList01[x] + inputStepList02[x])

    print(min(stepBothLines))
